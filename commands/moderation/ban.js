...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if(!message.member.hasPermission('BAN_MEMBERS')) return message.reply(`:x: You don't have Ban members to use this command`)
    
    if(!message.guild.me.hasPermission('BAN_MEMBERS')) return message.reply(`:x: You don't have Ban members to use this command`)
    if(!args.length) return message.reply(`> Usage: <prefix>ban <@user> <reason>`)
    ...
    if(target.bannable) {
      ...
      .setColor("RANDOM")
      .setDescription(`Banned \`${target.user.username}\` reason is \`${reason || "No Reason Provided"}\``)
